...
Slider::Slider(Vector2f bodySize, Vector2f sliderSize, Vector2f _position, float _min, float _max, string _text, Font &font)
{
	...
	body.setFillColor(Color::Blue) ; body.setOutlineColor(Color::Black) ; body.setOutlineThickness(1) ; 
	slider.setFillColor(Color::White) ;  slider.setOutlineColor(Color::Black) ; slider.setOutlineThickness(1) ; 
...
bool Slider::isSliderClicked(RenderWindow *window) 
{
	...
	if(!Mouse::isButtonPressed(Mouse::Left)) edit = false ;

	if (edit)
	{
		if(Keyboard::isKeyPressed(Keyboard::LControl)) slider.setPosition(slider.getPosition().x + (mousePos.x - prevMousePos.x)/100.f , slider.getPosition().y) ;
		else slider.setPosition(slider.getPosition().x + (mousePos.x - prevMousePos.x)/10.f , slider.getPosition().y) ;
		...
		if(slider.getPosition().x >= body.getPosition().x+body.getSize().x-slider.getSize().x) // boundings
