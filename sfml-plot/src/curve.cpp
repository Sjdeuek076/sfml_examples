...
// begin prepare coordinate for the curve
   // rangex and rangey are the values of asix   
    if(data_.size() > 1)
    {
        ...
        while(xoffset * data_.size() < (size_.x+xoffset))
            xoffset++;
// end prepare coordinate for the curve 
        ...
        for(std::list<float>::reverse_iterator it=data_.rbegin();it!=data_.rend();++it)
        {
            ...
            /* std::cout<<"xy: "<<x<<" , "<<y<<" ,size_.y: "<<size_.y<<" , size.x: "<<size_.x<<" ,realy: "<<realy<<" ,it: "<<*it
                     <<" ,distance, rangey.y, rangey.x: "<<distance<<" , "<<rangey.y<<" , "<<rangey.x
                     <<" , data_.size "<<data_.size()<<" , xoffset "<<xoffset<<std::endl;  */
